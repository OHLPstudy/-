...
1. 패키지는 오래 전부터 Java에 있었고, 모듈은 Java 9에서 추가된 개념이다. 자바 모듈 시스템에서 패키지와 모듈의 차이를 설명하고, 모듈이 패키지보다 더 나은 점과 모듈이 제공하는 장점에 대해 간략하게 서술하시오.
    ...
    - **캡슐화 강화** : 모듈은 패키지보다 캡슐화를 더 강화할 수 있다. 필요한 패키지만 외부에 노출하고, 나머지는 감출 수 있어 **보안성**과 **유지보수성**이 높아진다.
    - **명확한 의존성 관리** : 모듈 시스템을 통해 **모듈 간의 의존성을 명확히** 정의할 수 있다. 어떤 모듈이 다른 모듈에 의존하는지를 `requires` 키워드로 명시할 수 있어, 코드의 관계가 더 명확해진다.
    - **JVM 경량화** : 자바 프로그램을 배포할 때 필요한 모듈만 선택하여 포함할 수 있어 **경량화된 애플리케이션**을 만들 수 있다.
    
    #### 예시로 이해하기
    
    - 예를 들어, 패키지는 여러 개의 파일 폴더처럼 생각할 수 있다. 폴더마다 다른 클래스가 담겨 있고, 서로 연관된 파일들이 그룹화되어 있다.
    - 모듈은 이러한 패키지들을 한데 묶어 하나의 독립적인 소프트웨어 구성 단위로 만드는 것이라고 생각할 수 있다. 모듈을 통해, 필요한 파일(패키지)만 외부에 공개하고, 모듈 사이의 관계를 명확히 설정할 수 있다.
    
    이렇게 모듈은 패키지보다 더 강력한 구조화를 가능하게 해 주고, 프로그램의 크기를 줄이거나 코드 간의 관계를 명확히 정의할 때 유리하다.
