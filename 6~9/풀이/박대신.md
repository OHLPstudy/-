...
3.	**생성자**는 객체의 **초기화**를 담당하여, 올바른 객체 생성과 일관성을 유지합니다.  
    ...
    이 세 가지 구성 요소는 OOP의 기본 원칙인 캡슐화, 추상화, 다형성, 상속을 구현하는 데 필수적이며,  
    객체가 상태(필드)와 동작(메서드)을 가지는 기본 구조를 제공합니다.  
    이를 통해 실세계 모델링이 가능해지고, 유지보수성과 재사용성이 높은 프로그램을 설계할 수 있습니다.  
...
- 메서드 (Method)  
	 ...
	 •	객체는 메서드를 호출함으로써 상호작용하며, 이를 통해 클래스의 행동을 구현합니다.  
