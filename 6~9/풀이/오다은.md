...
- [정답 및 풀이]
	...
	*
	* 중첩 클래스란?
	*   - 외부 클래스와 관련된 작업을 논리적으로 묶어 관리하기 위한 도구
	* 정적 중첩 클래스란?
	*   - static 키워드로 선언된 중첩 클래스
	*   - 외부 클래스의 인스턴스와 독립적으로 동작
	*   - 외부 클래스의 정적 멤버와만 상호작용 가능
	*   - 외부 클래스의 객체 생성 없이 사용할 수 있음
	* 비정적 중첩 클래스란?
	*   - static 키워드가 없는 중첩 클래스
	*   - 외부 클래스의 인스턴스와 연결되어 동작
	*   - 외부 클래스의 모든 멤버(정적/비정적)에 접근 가능
	* 
	* 부모 클래스를 생성하지 않고 각 기능 별 클래스 생성
	*   - static으로 선언된 중첩 클래스는 외부 클래스의 인스턴스를 참조하지 않으며, 독립적으로 객체를 생성할 수 있음
	* 정적 중첩 클래스 사용 이유
	*   - 독립적으로 존재해야 하므로 static 중첩 클래스로 선언
	*   - 독립적으로 사용할 수 있으면서 LeeBoardDTO와 논리적 관계를 유지함
	*
	* 기능 별 DTO 분리를 하는 이유
	* 1. 기능 명확성
	*   - 각 기능 별 DTO 이름을 통해 해당 클래스가 어떤 용도로 사용하는지 명확하게 알 수 있음
	* 2. 필드 제약
	*   - Update 클래스에서 작성자와 조회수 필드를 제외헤여, 수정 시 해당 데이터가 입력되지 않도록 설계함 (Update에서는 id, title, content만 포함됨)
	* 3. 확장성
	*   - 새로운 기능이 추가될 경우, 기존 DTO에 영향을 주지 않고 필요한 DTO만 추가하거나 변경할 수 있어서 유지보수성이 높아짐.
	*
	* id Long Type 설정 이유
	* 1. 데이터베이스와 호환성
	*   - 데이터베이스에서 ID 필드는 보통 BIGINT 타입으로 정의됨
	*   - Java의 long 타입은 데이터베이스의 BIGINT 타입으로 매핑됨
	*   - int 타입은 데이터베이스의 INTEGER 타입과 매핑되며, 범위가 더 작아 대규모 시스템에서 사용할 때 제한적
	* 2. 확장성과 유지보수성을 고려한 설계
	* 3. 실무 표준에 부합
	* 
	* 그렇다면 int 타입을 사용하는 경우는?
	* - 데이터 개수가 매우 제한적인 시스템에 유효함 (간단한 애플리케이션, 테스트용 프로젝트, ID 값이 21억을 초과하지 않는 시스템)
	* 
	* Lombok 사용 이유
	* - 코드 간결화 및 유지보수성
	*   - @Data: Getter/Setter, toString(), equals(), hashCode() 메서드를 자동으로 생성
	*   - @AllArgsConstructor, @NoArgsConstructor 어노테이션을 사용하여 전체 필드를 초기화하는 생성자와 기본 생성자를 자동으로 생성
	*   - 수동으로 Getter/Setter 등을 작성할 필요가 없어, 코드 중복을 줄이고 유지보수를 간편하게 함
	* */
	```
